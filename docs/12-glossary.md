...
| `Resources` | 工具运行时的类型化异构依赖容器 | `TypeId -> Any` 的共享容器；承载 fs、cwd、client、`Params<T>`、`State<T>` 等，不能把它当作 session conversation | [`resources.rs`](../crates/codegen/xai-grok-tools/src/types/resources.rs)、[resources-and-capability-injection](./deep-dives/resources-and-capability-injection.md) |
| `SharedResources` | 可被多个 tool call 读取的资源句柄 | `Arc<Mutex<Resources>>`；锁只保护短暂读写，工具不能持锁跨网络/进程 await | [`resources.rs`](../crates/codegen/xai-grok-tools/src/types/resources.rs) |
| `ToolCallContext` | 单次工具调用的上下文 | 由 runtime 创建，包含 `call_id` 与 `TypedExtensions`；取消、trace 和 call-specific capability 属于这里 | [`context.rs`](../crates/common/xai-tool-runtime/src/context.rs) |
| shell `ToolContext` | session 基础设施上下文 | 名字与 runtime context 相近，但拥有 ACP、hunk、session 和 process scope；不是 Tool::run 的依赖注入容器 | [`tool_context.rs`](../crates/codegen/xai-grok-shell/src/tools/tool_context.rs) |
...
| `traceparent` | W3C 分布式追踪的父上下文字符串 | 通过 ACP `_meta`、SessionCommand 和 sampler HTTP header 传播 trace id；不是 `session_id` 或 `prompt_id` | [`trace_context.rs`](../crates/codegen/xai-file-utils/src/trace_context.rs)、[observability-and-trace-timeline](./deep-dives/observability-and-trace-timeline.md) |
| tracing span | 一段带父子上下文的工作范围 | `session.handle_prompt`、tool、sampler 等阶段用 span 记录结构化字段和耗时；event 继承当前 span | [`session_ctx.rs`](../crates/codegen/xai-grok-telemetry/src/session_ctx.rs)、[17-tracing](./rust-essentials/17-tracing.md) |
| unified log | 跨进程共享的 JSONL 生命周期账本 | shell 直接写，pager/desktop 通过 `x.ai/log` 转发；默认位于 `~/.grok/logs/unified.jsonl`，有 5 MiB trim | [`unified_log.rs`](../crates/codegen/xai-grok-telemetry/src/unified_log.rs)、[observability-and-trace-timeline](./deep-dives/observability-and-trace-timeline.md) |
| debug firehose | 按 tracing session span 路由的高密度文本日志 | `GROK_DEBUG_LOG=1` 生成每 session 文件；`GROK_LOG_FILE` 是单文件且遵守 `RUST_LOG` | [`debug_log.rs`](../crates/codegen/xai-grok-telemetry/src/debug_log.rs)、[observability-and-trace-timeline](./deep-dives/observability-and-trace-timeline.md) |
| `prompt_id` vs `turn_number` | 请求身份 vs conversation 顺序 | `prompt_id` 关联 ACP 请求和完成响应；`turn_number/prompt_index` 代表状态 owner 的 turn 边界，一个 turn 可有多个 HTTP attempt | [`commands.rs`](../crates/codegen/xai-grok-shell/src/session/commands.rs)、[`xai-chat-state`](../crates/codegen/xai-chat-state/src/actor/state.rs) |
| external OTEL | 用户明确选择的自有 collector 管道 | `GROK_EXTERNAL_OTEL` + signal exporter 双重 opt-in；独立于内部 xAI telemetry，内容 gate 默认关闭，exporter fail-closed | [`external/mod.rs`](../crates/codegen/xai-grok-telemetry/src/external/mod.rs)、[monitoring-usage](../crates/codegen/xai-grok-pager/docs/user-guide/24-monitoring-usage.md) |
| content gate | 是否允许发送 prompt/tool 细节的隐私闸门 | `OTEL_LOG_USER_PROMPTS` 和 `OTEL_LOG_TOOL_DETAILS` 只允许 additive opt-in；remote policy 只能关闭不能打开 | [`external/config.rs`](../crates/codegen/xai-grok-telemetry/src/external/config.rs)、[configuration-and-runtime-resolution](./deep-dives/configuration-and-runtime-resolution.md) |
| flush barrier | 退出/完成前等待异步写入落地的屏障 | `flush_blocking`、`drain_pending`、OTel shutdown 各自保护不同 sink；屏幕显示不等于日志或 session durable | [`unified_log.rs`](../crates/codegen/xai-grok-telemetry/src/unified_log.rs)、[persistence-and-replay](./deep-dives/persistence-and-replay.md) |
