...
不确定从哪一组开始时，先运行 `docs/rust-essentials/labs/study.sh tracks`；例如 `study.sh path turn` 会按认知依赖顺序运行 Session、Sampler、整轮 Turn、Pager 和 Replay 五个缩小实验。`study.sh checklist turn` 会输出对应精读和证据栏，但不会自动勾选；下面每节的“完成证据”仍需自己产出。
